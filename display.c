...
void envia_bits(uint8_t valor, uint8_t tipo) { // uint8_t para inteiro, positivo e 8 bits
	if (tipo) {
		PORTB |= (1 << PB0); // ? um dado, algo a ser mostrado
		} else {
		PORTB &= ~(1 << PB0); // ? um comando, limpar etc
	}

	PORTB &= ~((1 << PB2) | (1 << PB3) | (1 << PB4)); // inicia limpando
	PORTC &= ~(1 << PC3); // inicia limpando

	if (valor & 0x01) PORTB |= (1 << PB2); // loca??o bit 1
	if (valor & 0x02) PORTB |= (1 << PB3); // loca??o bit 2
	if (valor & 0x04) PORTB |= (1 << PB4); // loca??o bit 3
	if (valor & 0x08) PORTC |= (1 << PC3); // loca??o bit 4
...
void display_string(const char *texto) {
	while (*texto) {
		display_dado(*texto++); // digitador autom?tico
	}
}
...
void inicializa_display() {
	DDRB |= (1 << PB0) | (1 << PB1) | (1 << PB2) | (1 << PB3) | (1 << PB4); // configura as portas como sa?das
	DDRC |= (1 << PC3) | (1 << PC4);                                        // configura as portas como sa?das
	_delay_ms(50);
	...
	display_off();         // desliga temporariamente para o usu?rio n?o ver res?duos
